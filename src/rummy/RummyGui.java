            ...
            if(rp instanceof RummyHumanPlayer){
                if(draw == true){ // Draw
                    if(source == stockTop || source == discardPileTop) {
                        ((RummyGame)game).applyAction(new RummyMoveDraw(rp, 
                                (source == stockTop) ? DeckType.STOCK
                                        : DeckType.DISCARD));
                        move = true;
                        draw = false;
 ...
 |-----------------|
    ...
    protected void setGameMore(){
        ...
        for(int i = 0 ; i < numPlayers; i++) {
            this.playerNames[i] = new JButton("Player" 
                    + String.valueOf(i + 1));
    ...
    public void stateChanged(GameState newState){
    	...
    	for(int i = 0; i < numPlayers; i++){
            if(i == this.state.getCurrentPlayer() && ((RummyGame)game).currentIsHuman() == true) {
                ...
                for(swap = 0; swap < numPlayers; swap++) {
                    if(this.playerPosition[swap] == SOUTH){ // find player at SOUTH
                        break;
                    } 
                }
    	...
    	// Check for winner
    	for(int i = 0; i < numPlayers; i++ ){
            if( this.state.getHand( i ).isEmpty()){
     ...
     * @param text, as string, that represents cards (e.g. "C1 S12 D3 H5")
     * @return Vector<Card>
     */
    public Vector<Card> convertTextToVector(String text){
        String [] cardSequence = text.split(" ");
        Vector<Card> meld = new Vector<>();
