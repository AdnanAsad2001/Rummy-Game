...
public final class RummyDriverEngine {
     ...
     * @param options an array of string containing the list of legal choices
    ...
    private static String[] networkCancelStrArray = {"network client", "Cancel"};
    private static String[] networkNoMoreCancelStrArray = {"network client", "No more", "Cancel"};
    ...
    private RummyPlayer[] createLocalPlayers(RummyGame dummyGame) {
        ...
        Vector<RummyPlayer> playerVec = new Vector<>();
        ...
        for (;;) {
            ...
            if (playerVec.size() >= max){ 
                break;
            }
            // ask user what kind of game to play
            int response = getSelection("Player type ..." , "Player #"+(playerVec.size()+1) , menuStrings);
    ...
    */
    private String[] playerChoiceMenu(String[] extras) {
        ...
        String[] rtnVal = new String[localChoices.length+(extras.length)];
    ...
    /**
     * Notify the user that an error has occur.
     * @param message the text to display to the user
     */
    private void errorMessage(String message) {
        // pop up a dialog box with the message
        JOptionPane.showMessageDialog(null, message, "Please confirm",JOptionPane.ERROR_MESSAGE);
    }
}