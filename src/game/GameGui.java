...
public abstract class GameGui extends JFrame implements ActionListener, KeyListener, WindowListener {
     ...
     *
     * @return the GUI's initial height, in pixels.
     */
     ...
     *
     * @return the GUI's initial width, in pixels.
     */
     ...
     * @param where the container to which the button should be added
     * @param listener the listener that should listen to the button
     * @return the button created
     */
    protected JButton addButton(String text, Container where, ActionListener listener) {
    ...
    protected static void infoMessage(String message) {
        JOptionPane.showMessageDialog(null, message, "Please confirm", JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    protected final void submitAction(String actionType) {
		// create the event
        ActionEvent event = new ActionEvent(dummyButton, localEventCount++, actionType);
        // submit the event
     ...
     * which means the window (or one of its sub-components) will receive
     * keyboard events.
     *
     * @param e  the event object
     */
     ...
     * @return
     */
    public Color getBackground(Color BLUE) {
